...
public class Main {
	...
	static void respawnReplicaServers(Master master)throws IOException{
		...
		Master.masterLog.write(new Timestamp(new Date().getTime()) + ": [@main] respawning replica servers\n");
		Master.masterLog.flush();
		...
		for (int i = 0; i < n; i++) {
			...
			Master.masterLog.write(new Timestamp(new Date().getTime()) + ": replica server state [@ main] = "+rs.isAlive()+"\n");
			Master.masterLog.flush();
	...
	public static void launchClients(){
		try {
			Client c = new Client();
			char[] ss = "File 1 test test ".toCharArray();
			...
			c = new Client();
			ss = "File 1 Again Again ".toCharArray();
			...
			c = new Client();
			ss = "File 2 test test ".toCharArray();
			data = new byte[ss.length];
			for (int i = 0; i < ss.length; i++) 
				data[i] = (byte) ss[i];
	...
	static Master startMaster() throws IOException{
